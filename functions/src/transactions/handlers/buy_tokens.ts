...
export const buyTokens = functions.https.onCall(async (data, context) => {
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            const saldoFicticio =
                typeof userData.saldoFicticio === 'number' ? userData.saldoFicticio : 0;

            const tokenPrice =
                typeof startupData.tokenPrice === 'number' ? startupData.tokenPrice : 0;
            if (tokenPrice <= 0) {
                throw new functions.https.HttpsError(
                    'failed-precondition',
                    'Preco do token invalido na startup.'
                );
            }

            const availableTokens =
                typeof startupData.availableTokens === 'number' ? startupData.availableTokens : 0;
            const capitalRaised =
                typeof startupData.capitalRaised === 'number' ? startupData.capitalRaised : 0;
            ...
            if (availableTokens < quantity) {
                throw new functions.https.HttpsError(
                    'failed-precondition',
                    'Tokens insuficientes disponiveis na startup.'
                );
            }
            ...
            const currentTotalInvested =
                typeof positionData.totalInvested === 'number' ? positionData.totalInvested : 0;
            const newPositionQuantity = currentPositionQuantity + quantity;
            const newTotalInvested = currentTotalInvested + totalValue;

            const startupCategories = Array.isArray(startupData.categories)
                ? startupData.categories
                : Array.isArray(startupData.categorias)
                    ? startupData.categorias
                    : [];
            const startupSector =
                startupData.sector || (startupCategories.length > 0 ? startupCategories[0] : "");
            const startupName = startupData.name || "";
            ...
            transaction.set(
                positionRef,
                {
                    ...
                    averagePrice: newTotalInvested / newPositionQuantity,
                    updatedAt: now,
            ...
            transaction.set(transactionRef, {
                ...
                description: `${startupName || "Startup"} - ${quantity} token(s)`,
                startupId: normalizedStartupId,
                startupName: startupName,
                sector: startupSector,
                quantity: quantity,
